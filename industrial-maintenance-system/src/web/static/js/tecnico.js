...
function renderOrders() {
  ...
  // Helper: cleans and normalizes a name (removes accents, titles like Tec./Ing., and spaces)
  function normalizeTech(str) {
    if (!str) return '';
    return str
      .toLowerCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // remove accents (é -> e, etc.)
      .replace(/\b(ing|tec|tecnico|ingeniero|dr|lic)\.?\b/g, '') // remove titles
  ...
  function isMine(w) {
    ...
    const assignedRaw = w.assigned_technician.trim().toLowerCase();
    const myFullNameRaw = (user?.full_name || '').trim().toLowerCase();
    const myUsername = (user?.username || '').trim().toLowerCase();

    // 1. Direct match (exact or username)
    if (assignedRaw === myFullNameRaw || assignedRaw === myUsername) return true;

    // 2. Normalized match (e.g. "Tec. Laura Ramos" vs "laura ramos" vs "Tec Laura")
    const normAssigned = normalizeTech(assignedRaw);
    const normFullName = normalizeTech(myFullNameRaw);
    const normUsername = normalizeTech(myUsername);

    if (normAssigned && (normAssigned === normFullName || normAssigned === normUsername)) {
      return true;
    }

    // 3. Substring inclusion (e.g. "laura" included in "Tec. Laura Ramos")
    if (normFullName.length >= 3 && normAssigned.includes(normFullName)) return true;
    if (normAssigned.length >= 3 && normFullName.includes(normAssigned)) return true;
    if (normUsername.length >= 3 && normAssigned.includes(normUsername)) return true;
