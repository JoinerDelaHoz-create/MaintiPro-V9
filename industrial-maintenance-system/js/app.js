...
async function startWorkOrder(id) {
  try {
    if (window.CmmsStorage && window.CmmsStorage.startWorkOrder) {
      await window.CmmsStorage.startWorkOrder(id);
    }
    try {
      await apiRequest(`/api/work-orders/${id}/start`, { method: 'POST', silent: true });
    } catch (e) {
      // Silent fallback
    }
    ...
    showToast('Error', e.message || 'No se pudo iniciar la OT', 'error');
  }
}
...
async function cancelWorkOrder(id) {
  ...
  try {
    if (window.CmmsStorage && window.CmmsStorage.cancelWorkOrder) {
      await window.CmmsStorage.cancelWorkOrder(id);
    }
    try {
      await apiRequest(`/api/work-orders/${id}/cancel`, { method: 'POST', silent: true });
    } catch (e) {
      // Silent fallback
    }
    ...
    showToast('Error', e.message || 'No se pudo cancelar la OT', 'error');
  }
}
...
async function triggerTelemetrySimulation(injectAnomaly = false) {
  ...
  const code = (eqSelect && eqSelect.value) || (state.equipments[0] && state.equipments[0].code) || 'CMP-001';
  ...
  try {
    ...
    if (res && res.reading) {
      ...
      woId = res.work_order_id;
    }
  } catch (e) {
    console.warn('[Telemetry] Backend API unavailable, using local telemetry engine:', e.message);
  }

  // Local Telemetry Engine Fallback
  if (!r) {
    if (injectAnomaly) {
      r = {
        equipment_code: code,
        temperature: parseFloat((87.5 + Math.random() * 8.0).toFixed(1)),
        vibration: parseFloat((8.2 + Math.random() * 3.5).toFixed(2)),
        current: parseFloat((36.0 + Math.random() * 10.0).toFixed(1)),
        sound_db: parseFloat((89.0 + Math.random() * 8.0).toFixed(1)),
        pressure_psi: parseFloat((145.0 + Math.random() * 25.0).toFixed(1)),
        timestamp: new Date().toISOString().replace('T', ' ').substring(0, 19),
      };
      severity = 'CRITICAL';
      try {
        const newWo = await window.CmmsStorage.createWorkOrder({
          equipment_code: code,
          title: `[ALERTA IOT] Anomalía Térmica y Vibración Alta en ${code}`,
          description: `Telemetría automatizada detectó sobretemperatura (${r.temperature}°C) y vibración (${r.vibration} mm/s). Se requiere intervención inmediata.`,
          type: 'CORRECTIVE',
          priority: 'CRITICAL',
        ...
        console.warn('Error local WO creation:', err);
      }
    } else {
      r = {
        equipment_code: code,
        temperature: parseFloat((58.0 + Math.random() * 12.0).toFixed(1)),
        vibration: parseFloat((1.2 + Math.random() * 2.0).toFixed(2)),
        current: parseFloat((15.0 + Math.random() * 8.0).toFixed(1)),
        sound_db: parseFloat((62.0 + Math.random() * 8.0).toFixed(1)),
        pressure_psi: parseFloat((90.0 + Math.random() * 10.0).toFixed(1)),
        timestamp: new Date().toISOString().replace('T', ' ').substring(0, 19),
      };
      severity = 'NORMAL';
    }
  ...
  if (severity === 'CRITICAL' || severity === 'WARNING') {
    ...
    diagDetail.textContent = `Alerta disparada por exceso en tolerancias sensoriales. Se generó automáticamente la OT Predictiva #${woId || 'N/A'}.`;
    ...
    document.getElementById('alarm-banner-desc').textContent =
      `Temperatura: ${r.temperature}°C, Vibración RMS: ${r.vibration} mm/s. Generada automáticamente Orden de Trabajo Predictiva #${woId || 'N/A'}.`;

    tempCard.classList.add('anomaly-active');
    vibCard.classList.add('anomaly-active');

    appendTelemetryLog(`[ALERTA ${severity}] ${r.equipment_code} -> Temp: ${r.temperature}°C, Vib: ${r.vibration} mm/s. Generada OT #${woId || 'N/A'}`, 'critical');
    showToast('Alerta Predictiva', `Se generó automáticamente la OT #${woId || 'N/A'} para el equipo ${r.equipment_code}.`, 'error');
